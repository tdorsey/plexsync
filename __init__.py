...
def create_app(config_name=None, main=True):
    ...
    if main:
        # Initialize socketio server and attach it to 
        # the message queue, so that everything works 
        # even when there are multiple servers or 
        # additional processes such as Celery workers 
        # wanting to access Socket.IO
        socketio.init_app(app, message_queue=app.config['SOCKETIO_MESSAGE_QUEUE'])
    else:
        # Initialize socketio to emit events through 
        # through the message queue Note that since 
        # Celery does not use eventlet, we have to be 
        # explicit in setting the async mode to not t
        # use it.
        socketio.init_app(None,message_queue=app.config['SOCKETIO_MESSAGE_QUEUE'],async_mode='threading')
        celery.conf.update(config[config_name].CELERY_CONFIG)
    return app

def make_celery(app, main=True):
    app.logger.debug(f"making celery with Main = {main}")
